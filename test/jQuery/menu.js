function menu_ok(id, name, newproduct) {
    //////////////////////////////////MYOWN/////////////////////////////////////
    //alert("do ok ,id:"+id+", name:"+name);
    if (name != "") {
        $("#titleSearch").text("　搜尋結果-" + name);
    } else {
        $("#titleSearch").text("");
    }
    var pageitem = $("#pageitem a");
    for (var i = 0; i < pageitem.length; i++) {
        //var name = $("#searchname").val();
        var pstart = 0;//parseInt($("#pstart").val())
        var pend = 0;//parseInt($("#pend").val())
        var myurl = $(pageitem[i]).attr("href") + "&Id=" + id + "&name=" + name + "&newproduct=" + newproduct;
        if ($("#pend").val() == "") {//結束價格沒輸入
            if ($("#pstart").val() == "") {
                //都沒輸入 不查詢價錢範圍
                //MySearchFunc(name, pstart, pend);
                myurl += "&pstart=" + pstart + "&pend=" + pend;
            } else {
        ...
        $(pageitem[i]).attr("href", myurl);
    }
    $("#pageitem").children().children().children(".active").children().attr("href", "javascript: void(0)");
    for (var i = 0; i < $("p.memo").length; i++) {
        var id = $($("p.memo")[i]).data("id");
        if ($($("p.memo")[i]).children("label").length == 0) {
            $($("p.memo")[i]).append("<label><a href='#' style='color: rgb(0, 148, 255);' onclick='openitem($(this))'>more</a></label>");
        }
    }
    /////////////////////////////////MYOWN//////////////////////////////////////
...
function PublicReset() {
    ...
    $('.add_to_cart').click(function () {
        if ($("#logstatus").val() == "0") {
            ...
            $.getJSON("AddToCart", { ProductId: ProductId }, function (res) {
                if (res == true) {
                    //setTimeout(function () { alert("加入購物車成功!!"); }, 1000);
                } else {
                    //alert("商品已在購物車!!");
                }
            });
